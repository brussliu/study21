...
function loadScenarioNewBook(): void {
  ...
  const lines = Array.from({ length: 45 }, (_, index) => {
    ...
    return `デモ単語${String(number).padStart(2, '0')}\tでもたんご${String(number).padStart(2, '0')}\t演示单词 ${number}`
  })
...
async function save(): Promise<void> {
  ...
  if (ok) {
    ...
    await router.push({ name: 'student-japanese-demo' })
  }
}
...
<template>
  <div class="jp-demo">
    <div class="jp-demo__head">
      <div>
        <h1 class="page-title">単語の新規登録</h1>
        <p class="page-sub">まとめて入力して、書籍と Unit へ順番に割り当てます。</p>
      </div>
      <span class="jp-demo__badge" data-demo-badge><AppIcon name="info" size="sm" /> デモ</span>
      <div class="jp-demo__actions">
        <button type="button" class="btn btn--secondary btn--sm" data-demo-back @click="cancel">
    ...
    <p class="jp-demo__notice" data-demo-notice>
      ...
      <span>
        ...
        登録しても実際のシステムには保存されません（仮データの中だけに追加されます）。
      </span>
    </p>
    ...
    <!-- ============ 手順 1: 単語を入力 ============ -->
    <section v-if="store.registerStep === 1" class="card" data-demo-step="1">
    ...
    <!-- 解析結果（手順 1 の続き） -->
    <section v-if="store.registerStep === 1 && store.parsedRows.length > 0" class="card" data-demo-parsed>
      <div class="card__header">
    ...
    <!-- ============ 手順 2: 書籍・Unit ============ -->
    <section v-if="store.registerStep === 2" class="card" data-demo-step="2">
      <div class="card__header">
      ...
      <div class="jp-demo-fieldgrid">
        ...
        <div class="jp-demo-section">
          ...
          <p v-if="store.registerBookMode === 'EXISTING'" class="alert alert--warning" data-demo-unknown-unit>
            ...
            （デモの仮データではすべて <code>Unit001</code> の形なので、この案内は出ません）
          </p>
        </div>
      </div>
    ...
    <!-- ============ 手順 3: 内容を確認 ============ -->
    <section v-if="store.registerStep === 3" class="card" data-demo-step="3">
      <div class="card__header">
    ...
    <!-- フッターの操作 -->
    <div class="jp-demo__head" style="justify-content: flex-end">
      <div class="jp-demo__actions" style="margin-left: 0">
        <button
          v-if="store.registerStep > 1" type="button" class="btn btn--secondary btn--sm"
          data-demo-prev @click="store.gotoRegisterStep(store.registerStep - 1)"
        >
          <AppIcon name="chevron-left" size="sm" /> 戻る
        </button>
        <button
          v-if="store.registerStep === 1" type="button" class="btn btn--primary btn--sm"
          data-demo-next-1 :disabled="!canGoStep2" @click="store.gotoRegisterStep(2)"
        >
          次へ（書籍・Unit の設定） <AppIcon name="chevron-right" size="sm" />
        </button>
        <button
          v-if="store.registerStep === 2" type="button" class="btn btn--primary btn--sm"
          data-demo-next-2 :disabled="!canGoStep3" @click="store.gotoRegisterStep(3)"
        >
          次へ（内容の確認） <AppIcon name="chevron-right" size="sm" />
        </button>
        <button
          v-if="store.registerStep === 3" type="button" class="btn btn--primary btn--sm"
          data-demo-save :disabled="saving || store.registerWords.length === 0" @click="save"
        >
          <AppIcon name="check" size="sm" /> {{ saving ? '登録しています…' : '登録する（デモ）' }}
        </button>
      </div>
    </div>

    <p v-if="saved" class="alert alert--info" data-demo-saved>デモデータに登録しました。</p>
  </div>
</template>
